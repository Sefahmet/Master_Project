...
import com.master_porject.WeightDecider.Decider;
...
public class GraphCreator {
    ...
    public static Graph<Node, Edge> readAndAddOrgEdges2Graph(String filename, GraphFeatures graphFeatures) throws IOException {
        ...
        while (edgeiterator.hasNext()){
            ...
            Edge edge = new Edge(true,u_id,v_id,length,Math.pow((v_elev-u_elev)/length,2),maxspeed_weight,0.0,p1,p2,u,v);
            graph.addEdge(u, v,edge);
            graph.setEdgeWeight(u,v, Calculator.defaultWeightCalculator(edge));

            Edge reverseedge = new Edge(true,v_id,u_id,length,Math.pow((u_elev-v_elev)/length,2),maxspeed_weight,0.0,p2,p1,v,u);
    ...
    private static java.util.List<Coordinate> reverseList(java.util.List<Coordinate> coords){
